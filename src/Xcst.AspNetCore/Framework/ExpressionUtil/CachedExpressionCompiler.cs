...
static class CachedExpressionCompiler {
   ...
   public static Func<TModel, TValue>
   Process<TModel, TValue>(Expression<Func<TModel, TValue>> lambdaExpression) =>
      Compiler<TModel, TValue>.Compile(lambdaExpression);

   static class Compiler<TIn, TOut> {

      static Func<TIn, TOut>?
      _identityFunc;

      static readonly ConcurrentDictionary<MemberInfo, Func<TIn, TOut>>
      _simpleMemberAccessDict = new();

      static readonly ConcurrentDictionary<MemberInfo, Func<object, TOut>>
      _constMemberAccessDict = new();

      static readonly ConcurrentDictionary<ExpressionFingerprintChain, Hoisted<TIn, TOut>>
      _fingerprintedCache = new();

      public static Func<TIn, TOut>
      Compile(Expression<Func<TIn, TOut>> expr) =>
      ...
      static Func<TIn, TOut>?
      CompileFromConstLookup(Expression<Func<TIn, TOut>> expr) {
      ...
      static Func<TIn, TOut>?
      CompileFromIdentityFunc(Expression<Func<TIn, TOut>> expr) {
      ...
      static Func<TIn, TOut>?
      CompileFromFingerprint(Expression<Func<TIn, TOut>> expr) {
      ...
      static Func<TIn, TOut>?
      CompileFromMemberAccess(Expression<Func<TIn, TOut>> expr) {
      ...
      static Func<TIn, TOut>
      CompileSlow(Expression<Func<TIn, TOut>> expr) =>
