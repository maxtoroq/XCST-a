...
public sealed class BindAttribute : Attribute {
   ...
   string[]
   _excludeSplit = Array.Empty<string>();

   string?
   _include;

   string[]
   _includeSplit = Array.Empty<string>();
   ...
   IsPropertyAllowed(string propertyName, ICollection<string> includeProperties, ICollection<string> excludeProperties) {

      // We allow a property to be bound if its both in the include list AND not in the exclude list.
      // An empty include list implies all properties are allowed.
      // An empty exclude list implies no properties are disallowed.

      var includeProperty = (includeProperties is null) || (includeProperties.Count == 0) || includeProperties.Contains(propertyName, StringComparer.OrdinalIgnoreCase);
   ...
   public bool
   IsPropertyAllowed(string propertyName) =>
      IsPropertyAllowed(propertyName, _includeSplit, _excludeSplit);

   static string[]
   SplitString(string original) {
