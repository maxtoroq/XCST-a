...
public class ModelStateDictionary : IDictionary<string, ModelState> {
   ...
   readonly IDictionary<string, ModelState>
   _innerDictionary;
   ...
   public bool
   IsValid => Values.All(modelState => modelState.Errors.Count == 0);

   public ICollection<string>
   Keys => _innerDictionary.Keys;

   public ICollection<ModelState>
...
#pragma warning restore CS8766, CS8601
   ...
   // For unit testing
   internal IDictionary<string, ModelState>
   ...
   public
   ModelStateDictionary(ModelStateDictionary dictionary) {
   ...
   public void
   Add(KeyValuePair<string, ModelState> item) =>
      _innerDictionary.Add(item);

   public void
   Add(string key, ModelState value) =>
      _innerDictionary.Add(key, value);

   public void
   AddModelError(string key, Exception exception) =>
   ...
   public bool
   Contains(KeyValuePair<string, ModelState> item) =>
   ...
   public void
   CopyTo(KeyValuePair<string, ModelState>[] array, int arrayIndex) =>
      _innerDictionary.CopyTo(array, arrayIndex);

   public IEnumerator<KeyValuePair<string, ModelState>>
   GetEnumerator() =>
      _innerDictionary.GetEnumerator();

   private ModelState
   GetModelStateForKey(string key) {
   ...
   static IEnumerable<KeyValuePair<string, TValue>>
   FindKeysWithPrefix<TValue>(IDictionary<string, TValue> dictionary, string prefix) {
   ...
   public void
   Merge(ModelStateDictionary dictionary) {
   ...
   public bool
   Remove(KeyValuePair<string, ModelState> item) =>
   ...
   public void
   SetModelValue(string key, ValueProviderResult value) =>
      GetModelStateForKey(key).Value = value;

   public bool
   TryGetValue(string key, [MaybeNullWhen(returnValue: false)] out ModelState value) =>
...
public class ModelError {
   ...
   public Exception?
   Exception { get; private set; }

   public string
   ErrorMessage { get; private set; }
   ...
   public
   ModelError(Exception exception, string? errorMessage)
      : this(errorMessage) {
   ...
   public
   ModelError(string? errorMessage) {
