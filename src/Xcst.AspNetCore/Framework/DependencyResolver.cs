...
static class DependencyResolverExtensions {
   ...
   public static TService?
   GetService<TService>(this IDependencyResolver resolver) where TService : class =>
      (TService?)resolver.GetService(typeof(TService));

   public static IEnumerable<TService>
   GetServices<TService>(this IDependencyResolver resolver) =>
      resolver.GetServices(typeof(TService)).Cast<TService>();
}

class DependencyResolver {

   static readonly DependencyResolver
   _instance = new();

   public static IDependencyResolver
   Current => _instance.InnerCurrent;

   internal static IDependencyResolver
   CurrentCache => _instance.InnerCurrentCache;

   public IDependencyResolver
   InnerCurrent { get; private set; }

   /// <summary>
   /// Provides caching over results returned by Current.
   /// </summary>
   internal IDependencyResolver
   InnerCurrentCache { get; private set; }
...
#pragma warning restore CS8618
   ...
   public static void
   SetResolver(IDependencyResolver resolver) =>
      _instance.InnerSetResolver(resolver);

   public static void
   SetResolver(object commonServiceLocator) =>
      _instance.InnerSetResolver(commonServiceLocator);

   [SuppressMessage("Microsoft.Design", "CA1006:DoNotNestGenericTypesInMemberSignatures", Justification = "This is an appropriate nesting of generic types.")]
   public static void
   SetResolver(Func<Type, object?> getService, Func<Type, IEnumerable<object>> getServices) =>
      _instance.InnerSetResolver(getService, getServices);

   public void
   InnerSetResolver(IDependencyResolver resolver) {

      this.InnerCurrent = resolver ?? throw new ArgumentNullException(nameof(resolver));
      this.InnerCurrentCache = new CacheDependencyResolver(this.InnerCurrent);
   }

   public void
   InnerSetResolver(object commonServiceLocator) {
   ...
   [SuppressMessage("Microsoft.Design", "CA1006:DoNotNestGenericTypesInMemberSignatures", Justification = "This is an appropriate nesting of generic types.")]
   public void
   InnerSetResolver(Func<Type, object?> getService, Func<Type, IEnumerable<object>> getServices) {
   ...
   sealed class CacheDependencyResolver : IDependencyResolver {
      ...
      readonly ConcurrentDictionary<Type, object?>
      _cache = new();

      readonly ConcurrentDictionary<Type, IEnumerable<object>>
      _cacheMultiple = new();

      readonly Func<Type, object?>
      _getServiceDelegate;

      readonly Func<Type, IEnumerable<object>>
      _getServicesDelegate;

      readonly IDependencyResolver
      _resolver;

      public
      CacheDependencyResolver(IDependencyResolver resolver) {
         _resolver = resolver;
   ...
   class DelegateBasedDependencyResolver : IDependencyResolver {
      ...
      public
      DelegateBasedDependencyResolver(Func<Type, object?> getService, Func<Type, IEnumerable<object>> getServices) {
...
static class MultiServiceResolver {
   ...
   internal static TService[]
   GetCombined<TService>(IList<TService> items, IDependencyResolver? resolver = null)
      ...
      var services = resolver.GetServices<TService>();
