...
class CachedDataAnnotationsModelMetadata : CachedModelMetadata<CachedDataAnnotationsMetadataAttributes> {
   ...
   public
   CachedDataAnnotationsModelMetadata(CachedDataAnnotationsModelMetadata prototype, Func<object?>? modelAccessor)
      : base(prototype, modelAccessor) { }

   public
   CachedDataAnnotationsModelMetadata(CachedDataAnnotationsModelMetadataProvider provider, Type? containerType, Type modelType, string? propertyName, IEnumerable<Attribute> attributes)
   ...
   protected override bool
   ComputeIsRequired() =>
      (this.PrototypeCache.Required != null) ? true
   ...
   ComputeSimpleDisplayText() {
      ...
      if (this.Model != null) {
         ...
         if (this.PrototypeCache.DisplayColumn != null
            ...
            var displayColumnProperty = this.ModelType.GetProperty(this.PrototypeCache.DisplayColumn.DisplayColumn, BindingFlags.Public | BindingFlags.IgnoreCase | BindingFlags.Instance);
            ValidateDisplayColumnAttribute(this.PrototypeCache.DisplayColumn, displayColumnProperty, this.ModelType);

            var simpleDisplayTextValue = displayColumnProperty!.GetValue(this.Model, new object[0]);
         ...
         if (TryGetEnumDisplayName(out var enumDisplayName)) {
   ...
   TryGetEnumDisplayName([NotNullWhen(returnValue: true)] out string? displayName) {
      ...
      if (type.IsEnum) {
         ...
         var enumField = type.GetField(this.Model.ToString());
   ...
   static void
   ValidateDisplayColumnAttribute(DisplayColumnAttribute displayColumnAttribute, PropertyInfo? displayColumnProperty, Type modelType) {
...
class CachedDataAnnotationsMetadataAttributes {
   ...
   DataType { get; protected set; }

   public DisplayAttribute?
   Display { get; protected set; }

   public DisplayColumnAttribute?
   DisplayColumn { get; protected set; }

   public DisplayFormatAttribute?
   DisplayFormat { get; protected set; }

   public DisplayNameAttribute?
   DisplayName { get; protected set; }

   public EditableAttribute?
   Editable { get; protected set; }

   public HiddenInputAttribute?
   HiddenInput { get; protected set; }

   public ReadOnlyAttribute?
   ReadOnly { get; protected set; }

   public RequiredAttribute?
   Required { get; protected set; }

   public ScaffoldColumnAttribute?
   ScaffoldColumn { get; protected set; }

   public UIHintAttribute?
   UIHint { get; protected set; }

   public
   CachedDataAnnotationsMetadataAttributes(Attribute[] attributes) {
...
class CachedDataAnnotationsModelMetadataProvider : CachedAssociatedMetadataProvider<CachedDataAnnotationsModelMetadata> {
   ...
   protected override CachedDataAnnotationsModelMetadata
   CreateMetadataFromPrototype(CachedDataAnnotationsModelMetadata prototype, Func<object?>? modelAccessor) =>
