...
namespace System.Web.Mvc {
   ...
   static class DataTypeUtil {
      ...
      CreditCardTypeName = DataType.CreditCard.ToString();

      static readonly string
      CurrencyTypeName = DataType.Currency.ToString();

      static readonly string
      DateTypeName = DataType.Date.ToString();

      static readonly string
      DateTimeTypeName = DataType.DateTime.ToString();

      static readonly string
      DurationTypeName = DataType.Duration.ToString();

      static readonly string
      EmailAddressTypeName = DataType.EmailAddress.ToString();

      internal static readonly string
      HtmlTypeName = DataType.Html.ToString();

      static readonly string
      ImageUrlTypeName = DataType.ImageUrl.ToString();

      static readonly string
      MultiLineTextTypeName = DataType.MultilineText.ToString();

      static readonly string
      PasswordTypeName = DataType.Password.ToString();

      static readonly string
      PhoneNumberTypeName = DataType.PhoneNumber.ToString();

      static readonly string
      PostalCodeTypeName = DataType.PostalCode.ToString();

      static readonly string
      TextTypeName = DataType.Text.ToString();

      static readonly string
      TimeTypeName = DataType.Time.ToString();

      static readonly string
      UploadTypeName = DataType.Upload.ToString();

      static readonly string
      UrlTypeName = DataType.Url.ToString();

      static readonly Lazy<Dictionary<object, string>>
      _dataTypeToName = new(CreateDataTypeToName, isThreadSafe: true);
      ...
      ToDataTypeName(this DataTypeAttribute attribute, Func<DataTypeAttribute, Boolean>? isDataType = null) {
         ...
         if (isDataType(attribute)) {
            ...
            var name = KnownDataTypeToString(attribute.DataType);
      ...
      static string?
      KnownDataTypeToString(DataType dataType) =>
         dataType switch {
      ...
      static Dictionary<object, string>
      CreateDataTypeToName() {
         ...
         foreach (DataType dataTypeValue in Enum.GetValues(typeof(DataType))) {
            ...
            if (dataTypeValue != DataType.Custom
               ...
               var name = Enum.GetName(typeof(DataType), dataTypeValue);
