...
namespace System.Web.Mvc {
   ...
   public class TempDataDictionary : IDictionary<string, object?> {
      ...
      TempDataSerializationKey = "__tempData";

      Dictionary<string, object?>
      _data;

      HashSet<string>
      _initialKeys = new(StringComparer.OrdinalIgnoreCase);

      HashSet<string>
      _retainedKeys = new(StringComparer.OrdinalIgnoreCase);
      ...
      static void
      RemoveFromDictionary<TKey, TValue, TState>(IDictionary<TKey, TValue> dictionary, Func<KeyValuePair<TKey, TValue>, TState, bool> removeCondition, TState state) {
      ...
      GetEnumerator() => new TempDataDictionaryEnumerator(this);
      ...
      void
      ICollection<KeyValuePair<string, object?>>.CopyTo(KeyValuePair<string, object?>[] array, int index) =>
         ((ICollection<KeyValuePair<string, object?>>)_data).CopyTo(array, index);

      void
      ICollection<KeyValuePair<string, object?>>.Add(KeyValuePair<string, object?> keyValuePair) {
         _initialKeys.Add(keyValuePair.Key);
         ((ICollection<KeyValuePair<string, object?>>)_data).Add(keyValuePair);
      }

      bool
      ICollection<KeyValuePair<string, object?>>.Contains(KeyValuePair<string, object?> keyValuePair) =>
         ((ICollection<KeyValuePair<string, object?>>)_data).Contains(keyValuePair);

      bool
      ICollection<KeyValuePair<string, object?>>.Remove(KeyValuePair<string, object?> keyValuePair) {
      ...
      private sealed class TempDataDictionaryEnumerator : IEnumerator<KeyValuePair<string, object?>> {
         ...
         public
         TempDataDictionaryEnumerator(TempDataDictionary tempData) {
            _tempData = tempData;
